...
import { DEFAULT_TABLE_CONFIG, isTableValueEmpty, type TableConfigInput } from "@/lib/templateField";
...
function CellValue({ value, field }: { value: unknown; field: ReportField }) {
  if (field.type === "table") {
    if (isTableValueEmpty(value)) {
...
function InlineValue({ value, field }: { value: unknown; field: ReportField }) {
  if (field.type === "table") {
    if (isTableValueEmpty(value)) {
      return <span className="text-gray-400 dark:text-gray-500 italic">No answer</span>;
    }
    return (
      <TableFieldDisplay
        config={field.tableConfig ?? DEFAULT_TABLE_CONFIG}
              ...
              {visibleFields.map((f) => (
                <div key={f.id}>
                  ...
                  <div className="text-sm text-gray-700 dark:text-gray-300">
                    <InlineValue value={report.data[f.id]} field={f} />
                  </div>
                </div>
              ))}
              ...
              {!expanded && previewField && (
                <div className="flex-1 min-w-0 hidden sm:block">
                  ...
                  <span className="text-xs text-gray-600 dark:text-gray-400 line-clamp-1">
                    <InlineValue value={report.data[previewField.id]} field={previewField} />
                  </span>
                </div>
              )}
